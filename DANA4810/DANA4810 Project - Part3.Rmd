...
4. **Repeated $k$-fold Cross-Validation (Lowest Variance):**
   While plain $k$-fold CV balances bias and variance well, a single run on a small/moderate dataset can still be influenced by how observations happen to be partitioned across the $k$ folds. Repeating 10-fold CV 5 times (50 total resamples) does not change the level of bias, but it **further stabilizes and reduces the evaluation variance** of the performance estimate. Across the 50 resamples, our model demonstrated consistent stability with a low standard deviation in RMSE ($\text{SD} \approx 0.0463$) and $R^2$ ($\text{SD} \approx 0.1186$), offering the most reliable and robust assessment of model generalization.
